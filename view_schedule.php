...
require "config.php";
    ...
    if (isset($_POST['netid'])  && isset($_POST['term'])) {
        ...
        $stmt = $conn->prepare("select c.course_no,c.name,c.course_strength,c.instructor_id,c.course_time,c.room_no,c.start_date,c.end_date,c.course_code from courses c, enrolled_courses d where d.course_no = c.course_no and d.term_id = ? and d.netid = ?");
